...
if not isinstance(quizzes, list):
    raise ValueError("JSON must contain a ''quizzes'' array.")
```', NULL, '["Python の型注釈は実行時に型を強制するため、isinstance は冗長だが可読性のために書いている", "json.load() の戻り値は Any で静的に型が確定せず、型注釈だけでは実行時の保証がないため、isinstance で実際の型を検証する必要がある", "isinstance は型注釈を自動的に更新するために使われている", "data.get() が常にリストを返すことを確認するための形式的なチェック"]'::jsonb, 1, 'Python の型注釈はあくまで開発者とエディタへのヒントであり、実行時には型を強制しない。json.load() が返す Any 型のデータが期待通りのリストであることを保証するには、isinstance() による実行時チェックが不可欠。これにより不正な JSON に対して分かりやすい ValueError を返せる。', 'scripts/generate_migration.py', 'unpublished', false)
