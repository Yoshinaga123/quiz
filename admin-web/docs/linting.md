...
## TypeScript コンパイラ設定（tsconfig.app.json）

`tsconfig.app.json` も同様に厳格化済みです。
...
| `noUncheckedSideEffectImports` | `true` | 副作用 import の未チェックを検出 |
| `erasableSyntaxOnly` | `true` | 型消去できない構文（enum 等）を禁止 |
