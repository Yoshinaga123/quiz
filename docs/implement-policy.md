...
- 試行コードはルート `play.ts`、`packages/web/scripts/`、`packages/admin-web/scripts/`、`packages/backend/play.go` に置く（[Zod の `play.ts`](https://github.com/colinhacks/zod/blob/main/play.ts) 相当）。`src/schemas/` に残さない
- Zod スキーマ変更は成功・失敗の両方のテストを更新する（`packages/web/tests/schemas/`、`packages/admin-web/tests/schemas/`）。テストのない検証は未完成とする
