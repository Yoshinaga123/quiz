...
実装: `admin-web/src/schemas/quiz.ts`  
テスト: `admin-web/tests/schemas/quiz.test.ts`  
試し書き: `admin-web/scripts/`（本番スキーマに残さない）

公開 API の `quizSchema`（`web/src/schemas/quiz.ts`）とは別物。管理画面は `status` / `pushEnabled` / `createdAt` / `updatedAt` を持つ。これらのフィールドを `publicQuiz` に出してはならない。
