# ADR 0005: User-Facing Web Quiz App (`packages/web/`)
...
- `packages/admin-web/`: 管理画面（CRUD）
- `packages/mobile/`: モバイル向けクイズアプリ（Flutter, 雛形のみ）
...
ユーザー向け Web クイズアプリを `packages/web/` に新規追加する。
管理画面 `packages/admin-web/` とは **物理的に別プロジェクト** として運用し、依存とビルドを完全に分離する。

採用スタックは ADR 0002 / ADR 0003 に揃え、`packages/admin-web/` の規約をそのまま継承する。
...
- 本 ADR 決定時点では `packages/backend/main.go` に公開クイズ取得 API がなかった。
- そのため `packages/web/` を **インラインの starter pack（`src/data/quizzes.ts`）** で立ち上げ、後から `loadQuizzes()` の中身だけを fetch + zod 検証へ差し替えられる構造にした。
- その後 [ADR 0006](./0006-public-quiz-api.md) で `GET /v1/quizzes` 等が実装済みだが、`packages/web/` の `useQuizCatalog` は starter pack 同期取得のまま。クライアント側 API 統合は後続イテレーションとする。
...
- ESLint 設定、`tsconfig.*`、Tailwind v4 のテーマ変数など、`packages/admin-web/` と同一構成にすることで、レビュー基準とオンボーディングコストを共通化する。
...
- 履歴の永続化先を後から差し替え可能（`packages/web/src/lib/historyStorage.ts` に閉じている）

### Negative

- 候補プール（`packages/admin-web/src/data/quizzes.json`）と `packages/web/src/data/quizzes.ts`（starter pack）でデータが二重化している。公開 API（`/v1/quizzes`）は backend 側で提供済みだが、`packages/web/` がまだ呼んでいないため、次フェーズで以下のいずれかに統一する必要がある:
  1. `packages/web/` を `fetchQuizzes()` 経由で Public API から取得する（推奨）
...
| 2 | ✅ 完了 | `packages/web/src/api/quiz.ts` で `fetchQuizzes()` + zod 検証を実装 |
| 3 | ⏳ 未完了 | `packages/web/src/hooks/useQuizCatalog.ts` を非同期化し、`fetchQuizzes()` を呼ぶ |
| 4 | ⏳ 未完了 | ローディング・エラー状態を画面に伝播。`STARTER_QUIZZES` をフォールバックとして残すか決める |

`packages/web/.env.example` の `VITE_API_BASE_URL` に backend の URL（例: `http://localhost:8082`）を設定すると API 統合の準備が整う。
