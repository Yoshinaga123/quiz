...
`packages/admin-web/` と同じく React 19 + Vite 7 + TypeScript 5.9 + Tailwind CSS v4 + zod ^4 で構成し、規約・ESLint 設定は `packages/admin-web/` と揃えている。

## 役割

- IT 系の英文読解・コードの意味を問うクイズの **解答 UI** を提供する
- セッションごとの正答数・正答率・履歴を **ローカル** に保存する
- 管理画面 (`packages/admin-web/`) は CRUD のみ、こちらは回答体験のみ、と責務を完全分離する

## 現状（Public API 接続）

`packages/backend` は `GET /v1/quizzes` を提供している（[ADR 0006](../../docs/adr/0006-public-quiz-api.md)）。
`VITE_API_BASE_URL`（[`packages/web/.env.example`](.env.example) 参照）が設定されている場合、`useQuizCatalog` は `fetchQuizzes()` 経由で `/v1/quizzes` を取得する。
...
```bash
# 依存関係はリポジトリルートで `npm i`（workspaces）
