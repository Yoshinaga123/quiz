...
-- Existing DBs created before status/push_enabled keep the old quizzes
-- definition. CREATE TABLE IF NOT EXISTS then skips, so add and backfill
-- the admin columns before the seed INSERT names them.
...
CREATE INDEX IF NOT EXISTS idx_push_deliveries_quiz_id ON push_deliveries (quiz_id);
...
''You must also add the public SSH key to your account on GitHub before you use the key to authenticate or sign commits.''', NULL, '["SSH キーを使用して認証やコミット署名を行う前に、秘密鍵を GitHub アカウントに追加する必要がある", "SSH キーを使用して認証やコミット署名を行った後に、公開鍵を GitHub アカウントに追加する必要がある", "SSH キーを使用して認証やコミット署名を行う前に、公開鍵を GitHub アカウントに追加する必要がある", "SSH キーを使用して認証やコミット署名を行うには、秘密鍵と公開鍵の両方を GitHub に追加する必要がある"]'::jsonb, 2, '''public SSH key'' が公開鍵、''before'' が『〜する前に』を意味する。秘密鍵はローカルに保持し、公開鍵のみ GitHub に登録する。誤訳選択肢は秘密鍵・公開鍵の取り違え、before/after の誤訳をそれぞれ含んでいる。', 'https://docs.github.com/en/authentication/connecting-to-github-with-ssh', 'unpublished', false),
  ...
  (257, 'セクション79: クイズ同期 & 管理画面運用', 'published フラグの役割', '`packages/admin-web/src/data/quizzes.json` の `published` の役割として最も適切なものはどれですか？', NULL, '["管理画面の見た目だけを調整する Tailwind 設定ファイル", "シード同期の対象にするかどうかを表すフラグ", "ログイン用 JWT の秘密鍵を保存する JSON", "React Router のルーティング定義ファイル"]'::jsonb, 1, '`published: true` の問題だけがシード SQL 生成と DB 同期の入力になる。下書きは同じ `quizzes.json` に `published: false` で残す。', 'https://vite.dev/guide/assets#the-public-directory', 'unpublished', false),
  (258, 'セクション79: クイズ同期 & 管理画面運用', 'generate_migration.py の責務', '`python3 scripts/generate_migration.py --input packages/admin-web/src/data/quizzes.json` の説明として正しいものはどれですか？', NULL, '["マイグレーション番号を自動採番して DB へ即時適用する", "JSON を読み取って `published: true` の問題から SQL テキストを stdout に出力する", "quizzes.json を React 用 JSX に変換する", "DB の既存クイズを JSON に書き戻す"]'::jsonb, 1, '`generate_migration.py` は seed JSON を読み取り、`published: true` の問題だけから `INSERT ... ON CONFLICT ...` などの SQL テキストを標準出力へ出す。ファイル作成や採番は別処理で行う。', 'https://vite.dev/guide/assets#the-public-directory', 'unpublished', false),
  ...
  (262, 'セクション79: クイズ同期 & 管理画面運用', '管理画面ボタンの呼び先', '管理画面の `quizzes.json を DB 反映` ボタンを押したとき、最初にフロントが呼ぶ API はどれですか？', NULL, '["`POST /api/admin/quizzes/sync-production`", "`GET /api/admin/quizzes`", "`PATCH /api/admin/quizzes/{id}/status`", "`POST /api/admin/login/verification`"]'::jsonb, 0, '一覧画面の同期ボタンは `syncProductionQuizzes()` を呼び、認証付きで `POST /api/admin/quizzes/sync-production` を送る。', 'https://vite.dev/guide/assets#the-public-directory', 'unpublished', false),
  (263, 'セクション79: クイズ同期 & 管理画面運用', '同期 API が読むファイル', '管理画面の同期 API が既定で読み込むファイルパスはどれですか？', NULL, '["`packages/admin-web/src/data/quizzes.json`", "`packages/backend/seeds/quizzes.production.json`", "`packages/backend/migrations/006_seed_quizzes.up.sql`", "`docs/quiz-data-workflow.md`"]'::jsonb, 0, 'バックエンドは既定で `packages/admin-web/src/data/quizzes.json` を読み、`published: true` の問題だけをシードする。', 'https://vite.dev/guide/assets#the-public-directory', 'unpublished', false),
  (264, 'セクション79: クイズ同期 & 管理画面運用', 'replace モードの意味', '管理画面ボタンの同期仕様が replace モードに変更された後、正しい説明はどれですか？', NULL, '["JSON にある ID だけ追加し、JSON にない ID はそのまま残す", "JSON にある ID は Upsert し、JSON にない ID は DB から削除する", "JSON の内容は無視して常に DB 全件を保持する", "JSON にない ID だけを別テーブルへ移動する"]'::jsonb, 1, 'replace モードでは `published: true` の ID を正とみなし、それに含まれない ID を削除することで DB とシード対象を一致させる。', 'https://vite.dev/guide/assets#the-public-directory', 'unpublished', false),
  (265, 'セクション79: クイズ同期 & 管理画面運用', '空配列での replace 同期', '`quizzes.json` のシード対象（`published: true`）が 0 件だった場合の挙動はどれですか？', NULL, '["422 で拒否され、DB は変更されない", "`quizzes` テーブルが全件削除される", "最新 1 件だけが残る", "`schema_migrations` テーブルだけが削除される"]'::jsonb, 1, 'replace モードではシード対象を正として扱うため、`published: true` が 0 件なら DB 側の `quizzes` テーブルも空に同期される。', 'https://vite.dev/guide/assets#the-public-directory', 'unpublished', false),
  (266, 'セクション79: クイズ同期 & 管理画面運用', '同期時の status / push の扱い', '管理画面ボタンの同期 API が `status` と `push_enabled` を扱う方法として正しいものはどれですか？', NULL, '["既存行でも毎回 `published` / `true` に上書きする", "既存行では保持し、新規行だけ `unpublished` / `false` で挿入する", "常に JSON から `status` と `push_enabled` を読む", "同期時には `status` と `push_enabled` を NULL にする"]'::jsonb, 1, '`quizzes.json` の `published` はシード対象の選別だけに使い、DB の `status` と `push_enabled` は既存行では保持し、新規行のみ既定値 `unpublished` / `false` で作成する。', 'https://vite.dev/guide/assets#the-public-directory', 'unpublished', false),
  (267, 'セクション79: クイズ同期 & 管理画面運用', '422 エラーの意味', '管理画面の同期 API が 422 を返すケースとして最も適切なものはどれですか？', NULL, '["JWT が期限切れで再ログインが必要なとき", "`quizzes.json` の JSON 構文が壊れている、またはクイズデータが不正なとき", "API サーバーが起動していないとき", "削除モーダルのスクロールが足りないとき"]'::jsonb, 1, '422 は入力データが妥当でないことを表す。今回の実装では、壊れた JSON、重複 ID、必須項目不足、`correctAnswerIndex` の範囲外などが該当する。', 'https://vite.dev/guide/assets#the-public-directory', 'unpublished', false),
