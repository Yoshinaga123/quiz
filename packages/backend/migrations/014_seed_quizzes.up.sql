...
`__fspath__` プロトコル（PEP 519）がこの問題に対して提供する解決策として最も適切なものはどれですか？', NULL, '["open() が受け付ける文字列をファイル名として有効なもののみに制限する", "str 型を廃止し、すべてのパスを Path オブジェクトで表現することを強制する", "PathLike を実装したオブジェクトだけが「パスである」と明示的に宣言でき、ただの文字列とパスを型レベルで区別できるようにする", "open() に渡す前にファイルの存在確認を自動で行い、存在しなければ TypeError を発生させる"]'::jsonb, 2, 'str 型はあらゆる文字列を表すため、パスとして意図していない値でも open() に渡せてしまう。PEP 519 の __fspath__ プロトコルにより、PathLike を実装したオブジェクトだけが「これはパスである」と型レベルで宣言できる。後方互換のため str/bytes も引き続き受け付けるが、新しいコードでは Path を使うことで意図を明示できる。', 'https://peps.python.org/pep-0519/', 'unpublished', false),
...
```', 'while True:
    try:
        ...
        print("Oops! That was no valid number. Try again...")', '["`int(...)` が成功し、例外が発生しなかったとき", "`ValueError` が起きた直後", "ユーザーが Ctrl+C を押したとき", "入力が空文字だったときは必ず `break` する"]'::jsonb, 0, '`break` は try 節の中にあり、`int(input(...))` が正常に整数へ変換できた場合だけ到達する。`ValueError` や `TypeError` が起きると except に飛ぶのでループは継続する。', 'samples/python_study/8.3_Handling_Exceptions/input_number_try_example.py', 'unpublished', false),
...
```', 'try:
    ...
    print(inst.args)', '["`(''spam'', ''eggs'')`", "`[''spam'', ''eggs'']`", "`''spam eggs''`", "`None`"]'::jsonb, 0, '例外インスタンスに渡した複数引数は `.args` にタプルで保存される。サンプルではその後 `x, y = inst.args` とアンパックしている。', 'samples/python_study/8.3_Handling_Exceptions/input_number_try_example.py', 'unpublished', false),
...
```', 'try:
    ...
    x, y = inst.args', '["`x` に `''spam''`、`y` に `''eggs''` が入る", "`x` に `(''spam'', ''eggs'')`、`y` に `None` が入る", "`x` に例外クラス、`y` に例外メッセージが入る", "アンパックできず `TypeError` になる"]'::jsonb, 0, '`.args` は 2 要素タプル `(''spam'', ''eggs'')` なので、そのまま順番に `x`, `y` へアンパックできる。', 'samples/python_study/8.3_Handling_Exceptions/input_number_try_example.py', 'unpublished', false),
...
```', 'try:
    ...
    print(''Could not convert data to an integer.'')', '["ファイルは開けたが、読み込んだ文字列を `int(...)` に変換できなかったとき", "ファイルそのものが存在しないとき", "`open()` が成功したときは必ず ValueError になる", "`strip()` は常に `OSError` を送出するので ValueError にはならない"]'::jsonb, 0, '`OSError` はファイル操作レベルの失敗、`ValueError` は値変換の失敗。サンプルは「開けない」「開けるが中身が整数ではない」を分けて処理している。', 'samples/python_study/8.3_Handling_Exceptions/input_number_try_example.py', 'unpublished', false),
...
```', 'for arg in sys.argv[1:]:
    try:
        ...
        print(arg, ''has'', len(f.readlines()), ''lines'')
        f.close()', '["その反復で try 節に例外が発生しなかったとき", "例外が発生した直後に自動で実行される", "`except OSError` が実行された後に必ず続けて実行される", "for ループが最後まで回り切ったあと 1 回だけ実行される"]'::jsonb, 0, 'try/except の `else` は「例外が起きなかったときだけ」実行される。`for ... else` とは別物で、このサンプルでは各ファイルを正常に開けた場合だけ行数を表示している。', 'samples/python_study/8.3_Handling_Exceptions/input_number_try_example.py', 'unpublished', false),
...
try:
    ...
    print(''Handling run-time error:'', err)', '["`ZeroDivisionError`", "`ValueError`", "`ArithmeticException`", "`RuntimeError`"]'::jsonb, 0, '`1 / 0` は Python では `ZeroDivisionError` を送出する。関数の中で起きても、呼び出し元の try/except まで例外が伝播するので捕捉できる。', 'samples/python_study/8.3_Handling_Exceptions/input_number_try_example.py', 'unpublished', false),
...
```', 'try:
    ...
    print(''GrandFather'')', '["`son`", "`Father`", "`GrandFather`", "どの except にも一致せず未処理例外になる"]'::jsonb, 0, '`Son` 自身に一致する最初の except が実行される。より具体的な例外を先に書いているため、親クラスのハンドラまで落ちない。', 'samples/python_study/8.3_Handling_Exceptions/exception_hierarchy_example.py', 'unpublished', false),
...
```', 'try:
    ...
    print(''Father'')', '["`GrandFather`", "`son`", "`Father`", "捕捉されず例外終了する"]'::jsonb, 0, '`Son` は `GrandFather` のサブクラスでもあるため、先に書かれた `except GrandFather` がマッチしてしまう。except は上から順に評価され、最初に一致したものだけが実行される。', 'samples/python_study/8.3_Handling_Exceptions/exception_hierarchy_example.py', 'unpublished', false),
...
```', 'for c in [GrandFather, Father, Son]:
    try:
        ...
        print(''Father'')', '["`GrandFather`, `GrandFather`, `GrandFather`", "`GrandFather`, `Father`, `son`", "`GrandFather`, `son`, `Father`", "`son`, `Father`, `GrandFather`"]'::jsonb, 0, '先頭の `except GrandFather` が親クラスとして 3 種類すべてに一致するため、後ろの `except Son` と `except Father` は到達しない。', 'samples/python_study/8.3_Handling_Exceptions/exception_hierarchy_example.py', 'unpublished', false),
  (308, 'セクション96: Python raise と再送出', 'raise ValueError の意味', '次の説明として正しいものはどれですか？

```python
raise ValueError
```', 'raise ValueError', '["例外クラスをそのまま渡しており、Python が引数なしで暗黙にインスタンス化して送出する", "`ValueError` という文字列を送出している", "`ValueError` は `BaseException` のサブクラスではないので実行時に SyntaxError になる", "この書き方では必ず `ValueError()` と `TypeError()` が同時に送出される"]'::jsonb, 0, 'raise には例外インスタンスだけでなく例外クラスも渡せる。クラスを渡した場合、Python はコンストラクタを引数なしで呼んで例外オブジェクトを作る。', 'samples/python_study/8.4_Raising_Exceptions/raising_exceptions_example.py', 'unpublished', false),
...
```', 'try:
    ...
    print(''An exception flew by!'')
    raise', '["現在処理中の同じ例外をそのまま再送出する", "`NameError` を `RuntimeError` に自動変換して送出する", "except 節の中で発生した新しい例外だけを送出する", "例外を握りつぶして正常終了に変える"]'::jsonb, 0, 'except 節の中で引数なし `raise` を使うと、今まさに捕捉している例外を再送出する。ログを出してから上位へ伝播させたいときによく使う。', 'samples/python_study/8.4_Raising_Exceptions/raising_exceptions_example.py', 'unpublished', false),
...
```', 'try:
    ...
    print(''An exception flew by!'')
    raise', '["メッセージを 1 回表示した後、`NameError` が再び未処理例外として表に出る", "print された時点で例外は消えるので正常終了する", "最終的に `ValueError` へ変換される", "`raise` 行は except 節の中では無視される"]'::jsonb, 0, '`print` でメッセージを出しただけでは例外は解決しない。引数なし `raise` により同じ `NameError(''HiThere'')` が再送出され、上位で捕捉されなければトレースバック付きで終了する。', 'samples/python_study/8.4_Raising_Exceptions/raising_exceptions_example.py', 'unpublished', false),
  (311, 'セクション96: Python raise と再送出', 'どんなものを raise できるか', 'サンプル冒頭の説明に沿うと、`raise` に渡せるものとして正しい組み合わせはどれですか？', '# raise には何を渡せるか？', '["例外インスタンス、または `BaseException` 系を継承した例外クラス", "任意の文字列と整数だけ", "関数オブジェクトだけ", "辞書やリストだけ"]'::jsonb, 0, 'Python の `raise` は、例外インスタンスか例外クラスを受け取る。クラスであれば `BaseException` のサブクラスである必要がある。', 'samples/python_study/8.4_Raising_Exceptions/raising_exceptions_example.py', 'unpublished', false),
...
```', 'try:
  ...
  (313, 'セクション97: Python 例外連鎖', 'During handling of the above exception の意味', '次のサンプルで `open(''database.sqlite'')` が失敗したあとに `RuntimeError` を送出すると、トレースバック中に現れる `During handling of the above exception, another exception occurred:` という表示の意味として最も適切なものはどれですか？', 'try:
    ...
    raise RuntimeError(''unable to handle error'')', '["最初の例外 (`OSError`) を処理している最中に、別の例外 (`RuntimeError`) が発生したという暗黙の例外連鎖を示す", "2 つの例外が完全に無関係に並行実行されたことを示す", "`RuntimeError` が `OSError` のサブクラスであることを示す", "Python が例外メッセージを自動で翻訳したことを示す"]'::jsonb, 0, 'これは implicit exception chaining の表示で、元の例外が `__context__` としてぶら下がっていることを表す。『最初の失敗を処理しようとしていたら、さらに別の例外が起きた』という文脈が保持される。', 'samples/python_study/8.5_exception_chaining/exception_chaining.py', 'unpublished', false)
