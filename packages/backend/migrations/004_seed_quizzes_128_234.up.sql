...
fmt.Printf("Alloc: %s\n", formatBytes(stats.Alloc))', '["OSからGoランタイムに割り当てられた総メモリ量", "現在ヒープに使用中のメモリ量（GCされていないオブジェクト）", "プログラム開始からの累計アロケーション量", "スタック領域の使用量"]', 1, '`Alloc` は現在ヒープ上で生きているオブジェクトが使用中のバイト数です。GCが走るたびに減少します。累計アロケーション量は `TotalAlloc`、OSから取得した総量は `Sys` です。', 'packages/backend/main.go - logMemoryStats() / https://pkg.go.dev/runtime#MemStats');
...
fmt.Printf("TotalAlloc（累計）: %s\n", formatBytes(stats.TotalAlloc))', '["`TotalAlloc` はGC後に減少し、`Alloc` は単調増加する", "`Alloc` はGC後に減少するが、`TotalAlloc` はプログラム開始からの累計で減少しない", "両者は常に同じ値を返す", "`TotalAlloc` はスタックとヒープの合計、`Alloc` はヒープのみ"]', 1, '`Alloc` はGCが走るとオブジェクトが回収されるため減少します。`TotalAlloc` はプログラム開始から確保されたバイト数の累計で、単調増加のみです。メモリ圧力を測るには `Alloc` を、アロケーション頻度を測るには `TotalAlloc` の変化量を見ます。', 'packages/backend/main.go - logMemoryStats() / https://pkg.go.dev/runtime#MemStats');
...
}', '["\"1500 B\"", "\"1.46 KB\"", "\"1.50 KB\"", "\"0.00 MB\""]', 1, '1500 >= 1024 なのでKB換算になります。`1500 / 1024 = 1.46484...` なので `"1.46 KB"` です。ループは `n = 1500/1024 = 1` で `1 < 1024` のため即終了し `exp=0`（K）になります。', 'packages/backend/main.go - formatBytes()');
INSERT INTO quizzes (id, section, title, question, code, options, correct_answer_index, explanation, source) VALUES (136, 'セクション54: Go ランタイム メモリ統計', 'BuckHashSys の用途', '`stats.BuckHashSys` が表すものはどれですか？', 'fmt.Printf("BuckHashSys: %s\n", formatBytes(stats.BuckHashSys))', '["バケットソートに使われるメモリ量", "プロファイリング用バケットハッシュテーブルが使用しているメモリ量", "ハッシュマップの全エントリのメモリ量", "GCのマークビットマップに使われているメモリ量"]', 1, '`BuckHashSys` はGoランタイムがpprof等のプロファイリングデータを管理するバケットハッシュテーブルに使用するメモリ量です。通常は数百KB以下で、アプリのメモリ問題の主因にはなりません。', 'packages/backend/main.go - analyzeNonHeapMemory() / https://pkg.go.dev/runtime#MemStats');
...
}', '["time.Time", "int64（Unix秒）", "uint64（Unixナノ秒）", "float64（Unix秒の小数）"]', 2, '`MemStats.LastGC` は `uint64` 型で、最後のGCが完了したUnixナノ秒を表します。`time.Unix(0, int64(stats.LastGC))` で `time.Time` に変換します。`time.Unix(sec, nsec)` の第1引数をゼロにし、第2引数にナノ秒を渡すのがポイントです。', 'packages/backend/main.go - analyzeGC() / https://pkg.go.dev/runtime#MemStats');
...
}', '["NumGC がゼロだとPauseTotalNsも必ずゼロになるから", "ゼロ除算でパニックが発生するのを防ぐため", "uint64 の除算ではゼロ除算が無視されるから", "NumGC はゼロになり得ないから"]', 1, 'Goでは整数のゼロ除算は実行時パニック（`runtime error: integer divide by zero`）になります。`stats.NumGC > 0` のガードで安全に除算しています。なお `PauseTotalNs` が0でも除算は問題なく0を返すため、ガードは `NumGC` のみで十分です。', 'packages/backend/main.go - analyzeGC()');
...
}', '["GCが5%の確率で実行される", "プログラム実行時間の5%をGCが消費している", "ヒープの5%がGC対象になっている", "GCが毎秒5回実行されている"]', 1, '`GCCPUFraction` は直近のGCサイクルでGCがCPU時間の何割を使ったかを表す `float64`（0〜1）です。`0.05` なら5%のCPU時間をGCが使用しています。5%超は高負荷の目安とされ、`GOGC` を上げる（GC頻度を下げる）か、アロケーション量を減らすことが推奨されます。', 'packages/backend/main.go - gcTuningReport() / https://pkg.go.dev/runtime#MemStats');
INSERT INTO quizzes (id, section, title, question, code, options, correct_answer_index, explanation, source) VALUES (140, 'セクション55: Go GC 統計', 'PauseNs 配列とインデックス計算', '以下のコードで最新のGCポーズ時間を取得するインデックス計算の意味はどれですか？', 'idx := int((stats.NumGC - uint32(i) - 1 + 256) % 256)', '["256サイクル分の循環バッファから最新順にインデックスを計算している", "ランダムなサンプリング位置を計算している", "256で割った余りを使って配列の境界外アクセスを防いでいるだけ", "NumGCを256進数に変換している"]', 0, '`PauseNs` は256要素の循環バッファで、インデックスは `NumGC % 256` の位置に最新値が入ります。`(NumGC - i - 1 + 256) % 256` で i=0が最新、i=1が一つ前…と逆順にアクセスできます。`+256` はアンダーフロー防止です。', 'packages/backend/main.go - analyzeGCPauses() / https://pkg.go.dev/runtime#MemStats');
...
p99 := pauses[len(pauses)*99/100]', '["浮動小数点演算を避けて整数インデックスを求めるため", "配列を256要素に正規化するため", "sort.Sliceが1-basedインデックスを使うため", "99番目の要素だけを取り出すため"]', 0, 'スライスのインデックスは整数なので `int(float64(len)*0.99)` より整数演算 `len*99/100` の方がシンプルです。ソート済み配列の `len*99/100` 番目が99パーセンタイル（下から99%の位置）に相当します。厳密な実装ではなく近似値ですが、GCポーズの傾向把握には十分です。', 'packages/backend/main.go - analyzeGCPauses()');
...
if gcPercent < 0 {
    ...
    fmt.Printf("GOGC: %d%%\n", gcPercent)
}', '["GCを一時的に無効化して値を読み取るため", "`SetGCPercent` は現在値を返すので `-1` で読み取り専用アクセスし、元の値に戻している", "GCを2回トリガーするため", "負の値を渡すとGCが強制実行されるため"]', 1, '`debug.SetGCPercent(n)` は新しい値を設定し**直前の値**を返します。`-1` を渡すとGCが無効化されてしまうため、返ってきた元の値を即座に再設定して副作用を打ち消しています。現在値だけを読み取る専用関数がないため、このパターンが慣用的です。', 'packages/backend/main.go - gcTuningReport() / https://pkg.go.dev/runtime/debug#SetGCPercent');
...
}', '["次のGCまでにヒープが2倍になると予測されている", "現在のヒープの2倍に達したときに次のGCが発動する", "GCが2サイクル後に実行される", "ヒープ効率が50%である"]', 1, '`NextGC` は次のGCが発動するヒープサイズの目標値です。デフォルトの `GOGC=100` では前回GC後のヒープサイズの2倍が `NextGC` になります。`NextGC/HeapAlloc = 2.0` は「現在の2倍になったらGC」という状態を表し、デフォルト動作です。', 'packages/backend/main.go - gcTuningReport() / https://pkg.go.dev/runtime#MemStats');
...
fmt.Printf("Forced GC Cycles: %d\n", stats.NumForcedGC)', '["強制GCの一部が失敗した", "自動GC（ランタイムによるトリガー）が発生している", "GCが無効化されている", "NumForcedGCのカウントにバグがある"]', 1, '`NumGC` は全GCサイクル数、`NumForcedGC` は `runtime.GC()` 等で明示的に呼んだGCの回数です。`NumForcedGC < NumGC` はランタイムが自動でGCを実行したサイクルが存在することを意味します。通常の運用では `NumForcedGC` はほぼゼロで、`NumGC` の大半は自動GCです。', 'packages/backend/main.go - analyzeGC() / https://pkg.go.dev/runtime#MemStats');
...
}', '["ゴルーチンの実行速度を計測するため", "ゴルーチンリークを検出するため", "チャネルのバッファサイズを確認するため", "OSスレッド数の上限を確認するため"]', 1, 'ゴルーチンが適切に終了していれば、全ワーカー完了後のゴルーチン数は起動前の値に戻るはずです。`final > initial` の場合、いずれかのゴルーチンが終了していない（リーク）可能性を示します。テストでは `goleak` パッケージが同様の検出を行います。', 'packages/backend/main.go - main()');
...
time.Sleep(100 * time.Millisecond)
final := runtime.NumGoroutine()', '["次のGCサイクルを発生させるため", "ランタイムがゴルーチンのクリーンアップを完了する時間を与えるため", "チャネルのバッファをフラッシュするため", "OSスレッドのスケジューリングを安定させるため"]', 1, '`done <- true` を送信しても、送信側ゴルーチンがスケジューラによって完全に終了・回収されるまでわずかな時間がかかります。即座に `NumGoroutine()` を呼ぶとまだ終了処理中のゴルーチンがカウントされ誤検知になることがあります。短いスリープでランタイムのクリーンアップを待ちます。', 'packages/backend/main.go - main()');
INSERT INTO quizzes (id, section, title, question, code, options, correct_answer_index, explanation, source) VALUES (149, 'セクション56: Go ゴルーチン管理', 'runtime.GOMAXPROCS の意味', '`runtime.GOMAXPROCS(0)` を呼び出したとき何が返りますか？', 'fmt.Printf("GOMAXPROCS: %d\n", runtime.GOMAXPROCS(0))', '["GOMAXPROCSを0に設定して以前の値を返す", "現在のGOMAXPROCS値を変更せず返す", "利用可能なCPUコア数を返す", "実行中のOSスレッド数を返す"]', 1, '`GOMAXPROCS(n)` はn>0なら値を設定して以前の値を返し、n=0なら設定を変更せず現在値を返します。読み取り専用アクセスに `0` を使うのは慣用パターンです。初期値は `runtime.NumCPU()` と同じです。', 'packages/backend/main.go - main() / https://pkg.go.dev/runtime#GOMAXPROCS');
...
for i := 0; i < 10; i++ {
    <-done
}', '["受信側が <-done を呼ぶ準備ができるまでワーカーはブロックされる", "ワーカーは done <- true を非同期で送信してすぐ終了する", "バッファなしチャネルへの送信は常にパニックになる", "メインゴルーチンが <-done を呼ぶ前にバッファに積まれる"]', 0, 'バッファなしチャネルは送受信が必ずランデブー（同期）します。送信側（ワーカー）は受信側（メインゴルーチン）が `<-done` を呼ぶまでブロックされます。これにより「10回 `<-done` を受け取る = 10ワーカーが全員完了」という同期が実現します。', 'packages/backend/main.go - main()');
...
fmt.Printf("ワーカー生成後: %d\n", afterSpawn)', '["1（ゴルーチンはまだ起動していない）", "10（ワーカーのみ）", "11（メイン + 10ワーカー）", "不定（スケジューラ次第）"]', 2, '`go func()` でゴルーチンを起動すると即座にスケジューラに登録されます。メインゴルーチン(1) + 10ワーカー = 11 が期待値です。ただし `NumGoroutine()` はランタイムの内部ゴルーチン（GC補助など）も含む場合があり、環境によって±数個の誤差がありえます。', 'packages/backend/main.go - main()');
...
return token.SignedString(s.jwtSecret)', '["発行から1時間", "発行から24時間", "発行から7日間", "無期限"]', 1, '`time.Now().Add(24 * time.Hour)` で現在時刻から24時間後を `ExpiresAt` に設定しています。`jwt.NewNumericDate` はGoの `time.Time` をJWT仕様の NumericDate（Unixタイムスタンプ）に変換します。', 'packages/backend/main.go - issueJWT()');
...
})', '["JWTの署名を事前検証するため", "Authorizationヘッダーが存在しない・形式が違う場合を早期リターンするため", "Base64デコードを行うため", "トークンの有効期限を確認するため"]', 1, '`Bearer ` プレフィックスがない場合はJWT自体が存在しないか形式が不正です。`jwt.ParseWithClaims` を呼ぶ前に早期リターンすることで不要なパース処理を省き、エラーメッセージも明確になります。', 'packages/backend/main.go - requireAuth()');
INSERT INTO quizzes (id, section, title, question, code, options, correct_answer_index, explanation, source) VALUES (154, 'セクション57: Go JWT 認証', '署名アルゴリズムの検証', '以下のキー関数で署名アルゴリズムを確認している理由はどれですか？', 'func(token *jwt.Token) (any, error) {
    ...
    return s.jwtSecret, nil
}', '["パフォーマンス最適化のため", "alg:none 攻撃など意図しないアルゴリズムによる検証バイパスを防ぐため", "HS256以外ではシークレットキーが不要なため", "jwt.ParseWithClaims がアルゴリズムを自動検出できないため"]', 1, '攻撃者がヘッダーの `alg` を `none` に書き換えると、ライブラリによっては署名検証をスキップします。キー関数内で期待するアルゴリズムを明示的に確認することで、このアルゴリズム混同攻撃を防止します。これはJWT利用時のセキュリティベストプラクティスです。', 'packages/backend/main.go - requireAuth()');
...
INSERT INTO quizzes (id, section, title, question, code, options, correct_answer_index, explanation, source) VALUES (156, 'セクション57: Go JWT 認証', 'handleLogin での認証情報比較', '以下の認証処理で `payload.Username != s.adminUser || payload.Password != s.adminPassword` の条件が真の場合、HTTPステータスコードはどれですか？', 'if payload.Username != s.adminUser || payload.Password != s.adminPassword {
    ...
    return
}', '["400 Bad Request", "401 Unauthorized", "403 Forbidden", "404 Not Found"]', 1, '認証情報が不正（ユーザー名・パスワードの不一致）は `401 Unauthorized` です。`403 Forbidden` は認証済みだがアクセス権限がない場合、`400 Bad Request` はリクエスト形式が不正な場合に使います。RFC 7235に基づき、認証失敗は401が正しい選択です。', 'packages/backend/main.go - handleLogin()');
...
}', '["`json.Marshal` より高速なため", "メモリに全データをバッファせず `http.ResponseWriter` に直接ストリーム書き込みできるため", "文字コードを自動変換するため", "`any` 型を受け取れるのは `Encoder` だけのため"]', 1, '`json.Marshal` は全データをメモリ上の `[]byte` に一度展開してから書き込みます。`json.NewEncoder(w).Encode` は `io.Writer`（ここでは `http.ResponseWriter`）に直接ストリーム出力するためメモリ効率が良いです。大きなレスポンスで特に有効です。', 'packages/backend/main.go - writeJSON()');
...
decoder.DisallowUnknownFields()', '["JSONのパース速度を向上させるため", "1MB超のリクエストボディによるメモリ枯渇・DoS攻撃を防ぐため", "r.Body をコピーして再利用できるようにするため", "Content-Length ヘッダーの検証をするため"]', 1, '`1<<20` は 1MB（1,048,576バイト）です。制限なしで `r.Body` を読むと、巨大なボディを送りつけるDoS攻撃でサーバーメモリを枯渇させられます。`LimitReader` で上限を設けることで安全にボディを処理できます。', 'packages/backend/main.go - decodeJSON()');
...
if err := decoder.Decode(dst); err != nil {
    return err
}', '["未知フィールドは無視されてデコード成功", "デコードエラーが返される", "未知フィールドのみ別の変数に格納される", "パニックが発生する"]', 1, 'デフォルトでは `json.Decoder` は未知フィールドを無視します。`DisallowUnknownFields()` を呼ぶと、構造体に対応するフィールドが存在しないJSONキーがあった場合にエラーを返します。タイポや意図しないフィールドを早期検出するためのバリデーション手段です。', 'packages/backend/main.go - decodeJSON()');
...
}', '["追加フィールドをキャプチャするため", "リクエストボディに複数のJSONオブジェクトが含まれていないか確認するため", "デコードのキャッシュをクリアするため", "r.Body を閉じるため"]', 1, '1回目の `Decode` 後に `io.EOF` 以外が返る場合、ボディにまだデータが残っています。`{...}{...}` のように複数のJSONオブジェクトを連結して送る攻撃や誤りを検出できます。正常なリクエストなら2回目の `Decode` は `io.EOF` を返します。', 'packages/backend/main.go - decodeJSON()');
...
}', '["Go 1.18（Generics導入時）", "Go 1.20", "Go 1.22", "Go 1.19"]', 2, '`http.Request.PathValue` と `ServeMux` のパターンマッチング（`{id}` 記法）はGo 1.22で標準ライブラリに追加されました。それ以前はパスパラメータの取得に `gorilla/mux` や `chi` などサードパーティのルーターが必要でした。', 'packages/backend/main.go - handleGetQuiz() / Go 1.22 release notes');
...
VALUES ($1, $2, $3, ...)
ON CONFLICT (id) DO NOTHING;', '["同じidが存在するとエラーが発生する", "同じidが存在する場合は既存行を上書きする", "同じidが存在する場合はINSERTをスキップして正常終了する", "同じidが存在する場合はNULLを挿入する"]', 2, '`ON CONFLICT (id) DO NOTHING` は競合（主キー重複など）が発生した場合にエラーを発生させずスキップします。冪等なシード処理に適しています。上書きしたい場合は `ON CONFLICT DO UPDATE SET ...`（UPSERT）を使います。', 'packages/backend/migrations/002_seed_quizzes.up.sql');
INSERT INTO quizzes (id, section, title, question, code, options, correct_answer_index, explanation, source) VALUES (167, 'セクション59: Go SQL & JSONB', 'RETURNING 句の活用', '以下のINSERT文で `RETURNING` 句を使っている理由はどれですか？', 'item, err := scanQuiz(s.db.QueryRow(`
    ...
    RETURNING
        ...
        created_at,
        updated_at
`, ...))', '["INSERTの実行確認のため", "INSERT後にSELECTを別途発行せずに挿入された行のデータを1回のクエリで取得するため", "トランザクションを自動コミットするため", "created_at のデフォルト値を上書きするため"]', 1, '`RETURNING` はINSERT/UPDATE/DELETEで変更された行のカラム値を返すPostgreSQL拡張です。`INSERT ... RETURNING id, created_at` とすることで、DB側で生成された `BIGSERIAL` のIDやデフォルト値 `NOW()` の `created_at` を別途SELECTなしで取得できます。', 'packages/backend/main.go - handleCreateQuiz()');
...
if err := rows.Err(); err != nil {
    ...
    return
}', '["rows.Closeのエラーを確認するため", "ループ中に発生したDBエラー（ネットワーク断など）を検出するため", "結果セットが空かどうかを確認するため", "rows.Nextの戻り値を再確認するため"]', 1, '`rows.Next()` がfalseを返した理由は「全行読み終わった」または「エラー発生」の2つです。`rows.Err()` でイテレーション中のエラーを確認しないと、DBサーバーとの通信断などで途中で切れた結果を正常として返してしまうバグが起きます。', 'packages/backend/main.go - handleListQuizzes()');
...
if err != nil {
    ...
    writeError(w, http.StatusInternalServerError, err.Error())
    return
}', '["QueryRowは必ずエラーを返すため", "レコードが見つからない場合と内部エラーを区別して適切なHTTPステータスを返すため", "sql.ErrNoRowsはpanicを防ぐためのセンチネル値のため", "errors.Isを使わないと型アサーションが失敗するため"]', 1, '`db.QueryRow` はレコードが0件のとき `sql.ErrNoRows` を返します。これを区別しないと「存在しないID」へのリクエストに `500 Internal Server Error` を返してしまいます。`errors.Is` でエラーの種類を判定し、`404 Not Found` と `500` を正しく使い分けます。', 'packages/backend/main.go - handleGetQuiz()');
INSERT INTO quizzes (id, section, title, question, code, options, correct_answer_index, explanation, source) VALUES (170, 'セクション59: Go SQL & JSONB', 'setval でシーケンスをリセット', 'マイグレーションのシードSQL末尾にある以下のSQL文の目的はどれですか？', 'SELECT setval(''quizzes_id_seq'', (SELECT MAX(id) FROM quizzes));', '["シーケンスを1にリセットして最初からIDを採番し直すため", "INSERT後にシーケンスの現在値を最大IDに合わせ、次の自動採番が重複しないようにするため", "quizzes_id_seqテーブルを初期化するため", "MAX(id)の値をログに出力するため"]', 1, '`BIGSERIAL` のシーケンスは通常INSERTのたびにインクリメントされますが、`id` を明示指定したINSERT（シードデータ）ではシーケンスが進みません。このため次の通常INSERTが既存IDと重複する可能性があります。`setval` でシーケンスを `MAX(id)` に合わせることで重複を防ぎます。', 'packages/backend/migrations/002_seed_quizzes.up.sql');
...
var migrationsFS embed.FS', '["実行時にファイルシステムからSQLを読み込むため", "ビルド時にSQLファイルをバイナリに埋め込み、デプロイ時に別途ファイルを配置不要にするため", "SQLファイルを暗号化するため", "マイグレーションを自動実行するため"]', 1, '`//go:embed` はビルド時に指定したファイルをGoバイナリに埋め込みます。`migrations/*.sql` をバイナリに含めることで、デプロイ先サーバーにSQLファイルを別途配置する必要がなくなります。`embed.FS` は組み込みファイルへの読み取り専用アクセスを提供します。', 'packages/backend/main.go / https://pkg.go.dev/embed');
...
return nil', '["ErrNoChange はフォーマットエラーのため無視できる", "全マイグレーション適用済みの場合 ErrNoChange が返り、これはエラーではなく正常状態のため", "ErrNoChange はGoの標準エラーではないため比較できないから", "m.Up() は常にエラーを返すため"]', 1, '`m.Up()` は全マイグレーションが既に適用されている場合（追加変更なし）に `migrate.ErrNoChange` を返します。これはエラーではなく「何もすることがない」という正常な状態です。サーバー再起動のたびに `Up()` を呼ぶ構成ではこの処理が必須です。', 'packages/backend/main.go - runMigrations()');
...
INSERT INTO quizzes (id, section, title, question, code, options, correct_answer_index, explanation, source) VALUES (176, 'セクション61: Go 環境変数 & サーバー構造体', 'getEnv のフォールバックパターン', '以下の `getEnv` 関数で環境変数が空文字列 `""` の場合、`fallback` が返されますか？', 'func getEnv(key, fallback string) string {
    ...
    return fallback
}', '["はい、空文字列は falsy として扱われ fallback が返る", "いいえ、空文字列は設定済みとして扱われ空文字列が返る", "os.Getenv は空文字列を返さない", "パニックが発生する"]', 0, '`value != ""` の条件なので、環境変数が設定されていても値が空文字列の場合は `fallback` が返ります。`os.Getenv` は未設定・空設定どちらも空文字列を返すため、この実装では「未設定」と「空文字列に設定」を区別しません。区別が必要なら `os.LookupEnv` を使います。', 'packages/backend/main.go - getEnv()');
...
}', '["グローバル変数より読み書きが速いため", "テスト時にモックDBや異なる設定を注入しやすく、グローバル状態を避けられるため", "Goではメソッドに引数を渡せないため", "フィールドは自動的にgoroutine-safeになるため"]', 1, '依存関係を構造体フィールドに持たせる「依存性注入（DI）」パターンです。グローバル変数と違い、テスト時に `server{db: mockDB}` のように差し替えやすく、並行テストでの競合も避けられます。ハンドラがすべてメソッドとして `*server` に紐づくためコンテキストも明確です。', 'packages/backend/main.go - server struct / main()');
...
if err := http.ListenAndServe(":8080", s.routes()); err != nil {
    log.Fatal(err)
}', '["initDB() の直後", "main() 関数が返るとき（サーバーが停止したとき）", "各HTTPリクエスト処理後", "GCが実行されたとき"]', 1, '`defer` は宣言した関数（ここでは `main`）が返るときに実行されます。`http.ListenAndServe` はサーバーが停止するまでブロックするため、通常は `db.Close()` は呼ばれません。サーバーがシャットダウンしたとき（エラーまたはシグナル）に `main` が返り、`defer db.Close()` が実行されます。', 'packages/backend/main.go - main()');
...
}', '["SQLインジェクションを防ぐため", "前後の空白のみのデータが「空」として正しく検出されるようにするため", "全角スペースを除去するため", "文字列を小文字に変換するため"]', 1, '`TrimSpace` なしで `"  "`（空白のみ）を `== ""` で比較すると空と判定されません。バリデーション前に `TrimSpace` を適用することで「空白のみ入力」を「空」として検出します。また `payload` はポインタ渡しなので `TrimSpace` 後の値が呼び出し元にも反映されます。', 'packages/backend/main.go - normalizeQuizPayload()');
...
}', '["Goの配列は1始まりのため", "インデックスは0始まりなのでlen(options)はインデックスとして無効な値のため", "len()が負の値を返すことがあるため", "Options が空の場合のみチェックするため"]', 1, 'Goのスライスインデックスは0始まりなので、有効な範囲は `0` 〜 `len-1` です。`CorrectAnswerIndex == len(options)` は1つ外（out of bounds）になります。`< 0` と `>= len` の両方をチェックすることで配列外アクセスによるパニックを防ぎます。', 'packages/backend/main.go - normalizeQuizPayload()');
...
writeJSON(w, http.StatusOK, items)', '["var items []quiz と全く同じため、どちらでもよい", "var で宣言するとnil スライスになりJSONで null になるが、make([]quiz, 0)は空配列 [] になるため", "make の方がappendのパフォーマンスが良いため", "nil スライスへの append はパニックになるため"]', 1, '`var items []quiz` は `nil` スライスを作るため `json.Marshal` すると `null` になります。`make([]quiz, 0)` は空（長さ0）の非nilスライスを作るため `[]` になります。クイズが0件のとき `null` ではなく `[]` を返す方がAPIクライアントにとって扱いやすいため、`make` を使っています。', 'packages/backend/main.go - handleListQuizzes()');
...
`, ..., codeValue, ...))', '["空文字列のSQLパラメータはエラーになるため", "DBスキーマでcodeはNULL許容（TEXT, not NOT NULL）なので、コードなしのクイズはNULLを格納するため", "PostgreSQLでは空文字列とNULLは同じ扱いのため", "nil を渡すとPostgreSQLが自動でDEFAULT値を使うため"]', 1, 'DBスキーマで `code TEXT`（`NOT NULL` なし）のため NULL が許容されます。コードブロックがないクイズで空文字列 `""` を格納するよりも `NULL` を格納する方が「値なし」の意味が明確です。Go の `nil` を `any` 型で渡すと `database/sql` が SQL の `NULL` に変換します。', 'packages/backend/main.go - handleCreateQuiz() / handleUpdateQuiz()');
...
)', '["StrictMode を使っているため", "document.getElementById と createRoot によりブラウザのDOMにReactツリーをマウントしており、サーバーではなくブラウザ上でレンダリングが行われるため", "StrictModeはサーバーでは動作しないため", "App コンポーネントをラップしているため"]', 1, '`document.getElementById` は `document` オブジェクトを参照しており、これはブラウザ環境のみに存在します。`createRoot` でブラウザのDOM要素にReactをマウントすることがCSRの本質です。SSRでは `hydrateRoot` を使い、サーバーで生成済みのHTMLにイベントを付与します。', 'packages/admin-web/src/main.tsx');
...
export function getAuthToken(): string | null {
  ...
  return window.localStorage.getItem(''quiz-admin-token'')
}', '["window を global に置き換える", "typeof window === ''undefined'' でサーバー実行時を判定してnullを返す", "localStorage を sessionStorage に置き換える", "useEffect 内でのみ localStorage を使い、それ以外では Cookie を使う"]', 1, '`typeof window === ''undefined''` はサーバーサイド（Node.js）では `true` になります。このガードを追加することでSSRビルドエラーを回避できます。より本格的な対応としては、Cookie-based認証に切り替えてサーバーサイドでもトークンを読めるようにする方法が推奨されます（Next.js の `cookies()` API等）。', 'packages/admin-web/src/auth/session.ts / Next.js migration');
...
}', '["fetch に渡す URL", "SWR がデータをキャッシュ・重複排除するためのキーで、同じキーを使うコンポーネントはキャッシュを共有する", "ローカルストレージの保存キー", "API のエンドポイントを自動検出するための型情報"]', 1, 'SWR の第1引数はキャッシュキーです。同じキーを持つ `useSWR` は複数のコンポーネントから呼ばれてもリクエストが1回に重複排除（dedup）されます。第2引数の fetcher 関数が実際のデータ取得を行うため、キーはURLである必要はありませんが、慣習的にAPIパスを使います。', 'packages/admin-web/src/hooks/useQuizzes.ts / https://swr.vercel.app/docs/getting-started');
...
const QuizFormPage = lazy(() => import(''./pages/QuizFormPage''))', '["ビルド出力に変化はない", "QuizListPage と QuizFormPage が別チャンクに分離され、該当ページへの遷移時に初めてダウンロードされる", "全ページが1つのチャンクにまとめられてバンドルサイズが増える", "lazy を使うと開発時のHMRが無効になる"]', 1, '`React.lazy` + 動的 `import()` により Vite がページ単位の別チャンクを生成します。実際のビルド出力では `QuizListPage-xxx.js`（261KB）と `QuizFormPage-xxx.js`（8KB）が分離され、初期バンドル `index-xxx.js`（304KB）には含まれません。ログインページを開いたときにクイズ関連のJSをダウンロードしなくて済み、初期表示が高速化します。', 'packages/admin-web/src/App.tsx');
