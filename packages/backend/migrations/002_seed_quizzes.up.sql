...
今回のケースは localhost 開発環境専用かつ Credentials なしのため実害はありませんが、本番コードに流用しないよう注意が必要です。', 'view-counter/packages/backend/main.go'),
...
# - packages/backend/ に Dockerfile は存在しない
